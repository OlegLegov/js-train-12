...
function convertMapToObject(map) {
  // Перетворюємо Map в об'єкт за допомогою spread оператора та Object.fromEntries
...
function transformDictionary(dictionary) {
  // Створюємо порожні масиви для ключів, значень і пар ключ-значення
  let keys = [];
  let values = [];
  let entries = [];
  ...
  for (let key of dictionary.keys()) {
    // Додаємо кожен ключ до масиву keys
    keys.push(key);
  }

  // Перебираємо значення словника
  for (let value of dictionary.values()) {
    // Додаємо кожне значення до масиву values
    values.push(value);
  }

  // Перебираємо пари ключ-значення словника
  for (let entry of dictionary.entries()) {
    // Додаємо кожну пару ключ-значення до масиву entries
    entries.push(entry);
  }
